...
class TaskBloc extends HydratedBloc<TaskEvent, TaskState> {
    ...
    if (event.task.isFavorite == true) {
      if (event.task.isDeleted == false) {
        ...
        List<Task> favoriteList = List.from(state.favoriteTasks)
          ..remove(event.task);
        event.task.isDone!
            ? favoriteList.insert(index, event.task.copyWith(isDone: false))
            : favoriteList.insert(index, event.task.copyWith(isDone: true));
        emit(TaskState(
  ...
  void _searchTaskEvent(SearchTaskEvent event, Emitter<TaskState> emit) {
    ...
    if (event.task.isFavorite == true) {
      if (event.searchValue != '') {
        List<Task> listTask = state.favoriteTasks;
  ...
  void _editTaskEvent(EditTextEvent event, Emitter<TaskState> emit) {
    if (event.task.isFavorite == true) {
      ...
      List<Task> list = List.from(state.favoriteTasks)
        ..insert(
            index,
            event.task.copyWith(
  ...
  void _favoriteTaskEvent(FavoriteTaskEvent event, Emitter<TaskState> emit) {
    ...
    if (event.task.isFavorite == false) {
      ...
      List<Task> favoriteList = List.from(state.favoriteTasks)
        ..remove(event.task);
      if (event.task.isDone == false) {
        pendingList.add(event.task.copyWith(isFavorite: false));
      } else {
        completedList.add(event.task.copyWith(isFavorite: false));
      }
      emit(TaskState(
          favoriteTasks: favoriteList,
          removedTasks: List.from(state.removedTasks),
